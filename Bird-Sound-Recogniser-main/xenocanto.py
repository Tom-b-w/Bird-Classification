...
def metadata(filt):
    ...
    # Only download the first page of metadata
    url = f'https://www.xeno-canto.org/api/2/recordings?query={query}&page={page}'
    try:
        r = request.urlopen(url)  # 不使用代理，直接打开 URL
    ...
    filename = f'{path}/page{page}.json'
...
def chunked_http_client(num_chunks):
    ...
    async def http_get(track_tuple, client_session):
        ...
        async with semaphore:
            ...
            folder_path = 'audio_origin/birds/'  # 直接保存到 audio_origin/birds/ 文件夹
            file_path = folder_path + name + '.wav'  # 使用中文文件名保存音频
            ...
            if os.path.exists(file_path):
                print(f"{name}.wav is already present. Skipping...")
                return
            ...
            async with client_session.get(url) as response:
                if response.status == 200:
                    ...
                    print(f"Error 503 occurred when downloading {track_id}.wav. Please try using a lower value for num_chunks. Consult the README for more information.")
                else:
                    print(f"Error {response.status} occurred when downloading {track_id}.wav.")
...
async def download(filt, num_chunks=4):
    # Retrieve metadata and generate list of track information
    meta_path = metadata(filt)  # metadata 函数已经限制为只下载第一页
    url_list = list_urls(meta_path)  # 解析第一页的数据
    ...
    # 初始化下载客户端（不使用代理）
    http_client = chunked_http_client(num_chunks)  # 不传 proxy_url

    # 可选：禁用 SSL 检查（通常不建议，除非必要）
    connector = aiohttp.TCPConnector(ssl=False)
    async with aiohttp.ClientSession(connector=connector) as client_session:
        # 只下载第一条音频
        first_track_tuple = url_list[1][0]
        await http_client(first_track_tuple, client_session)
...
def gen_meta(path='audio_origin/birds/'):
